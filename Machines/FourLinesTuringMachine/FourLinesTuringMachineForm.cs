...
{
    public partial class FourLinesTuringMachineForm : Form
    {
        Label[] labels = new Label[60];
        TuringMachine turingMachine;
        String PreviousAlphabet;
        System.Windows.Forms.Timer timer = new System.Windows.Forms.Timer();
        bool isOpenFile = false;

        public FourLinesTuringMachineForm()
        {
            InitializeComponent();
        ...
        private void ShowLine()
        {
            ...
            int pos4 = turingMachine.CurrentPos_Fourth - 7;
            ...
            for (int i = 45; i < 60; i++)
            {
                if (turingMachine.Line_Fourth[pos4] != 'λ')
                    labels[i].ForeColor = Color.Black;
                else
                    labels[i].ForeColor = Color.Gray;
                labels[i].Text = turingMachine.Line_Fourth[pos4].ToString();
                pos4++;
            }
        }
        ...
        private void OnLabelClick(object sender, EventArgs e)
        {
            ...
            if (getElemFromAlphabet.DialogResult == DialogResult.OK)
            {
                ...
                else if (label_num <= 59)
                {
                    int pos = turingMachine.CurrentPos_Fourth - 7 + label_num - 45;
                    char[] charArray = turingMachine.Line_Fourth.ToCharArray();
                    charArray.SetValue(getElemFromAlphabet.Letter, pos);
                    turingMachine.Line_Fourth = new string(charArray);
                    ShowLine();
                    return;
                }
            }
        ...
        struct PartsOfCommand
        {
            ...
            public char Direction4;
            public int PosDirection4;
            public int Num;
            public int PosNum;
        }
        ...
        private void ButtonToLeft_Click(object sender, EventArgs e)
        {
            ...
            if (sender == ButtonToLeftFourth)
                if (turingMachine.CurrentPos_Fourth + 7 != turingMachine.Line_Fourth.Length)
                    turingMachine.CurrentPos_Fourth++;
            ShowLine();
        }
